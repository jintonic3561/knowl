...
cat > .env <<'EOF'
GH_TOKEN="ghp_..."
# Slack 連携を使う場合のみ (詳細は後述)
SLACK_BOT_TOKEN="xoxb-..."
SLACK_CHANNEL="#通知したいチャンネル"
# Slack slash command `/knowl` も使う場合のみ追加
...
サマリ通知 / limit アラート、および slash command `/knowl` での ad-hoc 起動はどちらも 1 個の Slack App で賄える。使う機能に応じて手順を取捨選択する。
...
5. slash command `/knowl` も使う場合は追加で以下を実施
...
cron 周期を待たずに「今これやって」を投げたいとき用。常駐 knowl コンテナに Slack bot を相乗りさせ、Socket Mode で `/knowl` を受ける。HTTPS エンドポイントの公開は不要。セットアップは前節 [Slack 連携セットアップ](#slack-連携セットアップ) を参照。
